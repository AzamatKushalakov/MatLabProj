...
int main()
{
	...
	Matrix1 = mxCreateDoubleMatrix(row, col, mxREAL); // 1-а€ матрица

	double *Matrixx1 = mxGetPr(Matrix1);

	double Arrays[] = { 1, 3, 4, 2 };

	//  опируем мы тут потому из Arrays в Matrixx потому что так устроен матлаб 
	memcpy(Matrixx1, Arrays, row*col* sizeof(double));

	// ѕомещаем переменную Matrix1 в рабочую область matlab
	engPutVariable(Eg, "Matrix1", Matrix1);

	// —оздаем 2 матрицу и делаем все аналогично
	mxArray *Matrix2;
